...
test('evaluate', () => {	
	...
	expect(e.evaluate(p.parse("(cond ((eq 'a 'b) '(a b c)) ((set 'b 'c) '(a a a)))")))
		.toStrictEqual(p.parse("(a a a)"))
	...
	expect(e.evaluate(p.parse("(eval b)"))).toStrictEqual(p.parse('c'))
	expect(e.evaluate(p.parse("(eval (cond ((eq a 'a) 'c) ((eq a 'b) 'd)))")))
		.toStrictEqual(nil)
})
